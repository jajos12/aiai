...
export function ChallengeCanvas({ challenge, onComplete }: ChallengeCanvasProps) {
  ...
  // In a real implementation this would hook into the agent's episode loop.
  // We'll mock it for the visualizer.
  useEffect(() => {
    const timer = setTimeout(() => {
      // Simulate random performance that eventually hits target
      const mockPerformance = Math.floor(Math.random() * 10) + 15;
      setDistance(mockPerformance);

      if (mockPerformance <= threshold && !won) {
        setWon(true);
        setShowSuccess(true);
        onComplete();
      }
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      >
        <div>
          <span style={{ color: 'rgba(255,255,255,0.4)' }}>Avg Steps: </span>
