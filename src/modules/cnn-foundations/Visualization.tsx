...
import { Text, Float, RoundedBox, Box, Line, Wireframe } from '@react-three/drei';
...
export default function CNNVisualization({ kernelType = 'sobel-v' }: CNNVisualizationProps) {
        ...
        <group>
          ...
          <group position={[-5, 0, 0]}>
             {Array.from({ length: INPUT_SIZE * INPUT_SIZE }).map((_, i) => {
                ...
                const isActive = ix >= cursor.x && ix < cursor.x + KERNEL_SIZE && iy >= cursor.y && iy < cursor.y + KERNEL_SIZE;
                return (
             ...
             <Text position={[INPUT_SIZE/2, -1, 0]} fontSize={0.3} color="white">INPUT IMAGE</Text>
          </group>
          ...
          <group position={[-5 + cursor.x * 1.1, cursor.y * 1.1, 2]}>
             ...
             <Text position={[KERNEL_SIZE/2, KERNEL_SIZE + 0.5, 0]} fontSize={0.25} color="#ef4444">KERNEL</Text>
             
             {/* Dynamic connectivity lines */}
             <Line points={[[0, 0, 0], [5 + (cursor.x - (INPUT_SIZE/2)) * 1.1, 2, 2]]} color="#fbbf24" opacity={0.4} transparent />
          </group>
          ...
          <group position={[3, 0, 0]}>
             ...
             <Text position={[(INPUT_SIZE - KERNEL_SIZE + 1)/2, -1, 0]} fontSize={0.3} color="white">FEATURE MAP</Text>
          </group>
