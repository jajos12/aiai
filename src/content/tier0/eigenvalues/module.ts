...
const eigenvaluesModule: Module = {
  ...
  playground: {
    description: 'Full EigenTransform playground — explore any 2×2 matrix with its eigenvalues, eigenvectors, grid warp, and decomposition.',
    component: 'EigenTransform',
    parameters: [
      { id: 'showDotCloud', label: 'Show dot cloud', type: 'toggle', default: true },
      { id: 'showTransformedGrid', label: 'Show transformed grid', type: 'toggle', default: true },
      { id: 'showBasisVectors', label: 'Show basis vectors', type: 'toggle', default: true },
      { id: 'showEigenspaceLines', label: 'Show eigenspace lines', type: 'toggle', default: true },
      { id: 'showScalingIndicators', label: 'Show scaling indicators', type: 'toggle', default: false },
      { id: 'showCharacteristicEq', label: 'Show characteristic equation', type: 'toggle', default: false },
      { id: 'showTraceDetRelation', label: 'Show trace/det relation', type: 'toggle', default: false },
      { id: 'showDeterminantArea', label: 'Show determinant area', type: 'toggle', default: false },
      { id: 'showUnitCircle', label: 'Show unit circle', type: 'toggle', default: false },
      { id: 'showAnimation', label: 'Show animation controls', type: 'toggle', default: false },
      { id: 'showPowerIteration', label: 'Show power iteration', type: 'toggle', default: false },
      { id: 'showDecomposition', label: 'Show decomposition stages', type: 'toggle', default: false },
      { id: 'showMatrixControls', label: 'Show matrix controls', type: 'toggle', default: true },
      { id: 'showPresets', label: 'Show presets', type: 'toggle', default: true },
      { id: 'highlightEigenDots', label: 'Highlight eigen-dots', type: 'toggle', default: true },
    ],
    tryThis: [
      'Try a rotation matrix — eigenvalues become complex, no real eigenvectors!',
      'Make a symmetric matrix — eigenvectors are always perpendicular.',
      'Set [[2,1],[0,2]] — repeated eigenvalue but only one eigenvector (defective).',
      'Use power iteration to find the dominant eigenvector in < 5 steps.',
      'Enable decomposition and step through P⁻¹ → D → P.',
    ],
  },
  challenges: [
    {
      ...
      description: 'Set the matrix so its eigenvectors align with the directions (1, 1) and (1, -1).',
      component: 'EigenTransform',
      completionCriteria: { type: 'threshold', target: 0.1, metric: 'eigenvector_angle_error' },
      hints: [
        'The eigenvectors (1,1) and (1,-1) are perpendicular — so the matrix should be symmetric.',
        'Try a diagonal matrix in the rotated basis: think about what eigenvalues to assign.',
      ],
    },
    {
      id: 'make-rotation',
      title: 'Make It Rotate',
      description: 'Set the matrix so that both eigenvalues are complex (pure rotation, no real eigenvectors).',
      component: 'EigenTransform',
      completionCriteria: { type: 'custom', target: 'complex', metric: 'eigenvalue_type' },
      hints: [
        'The discriminant tr²-4det must be negative.',
        'A rotation matrix works: try [[0,-1],[1,0]] for a 90° rotation.',
      ],
    },
    {
      id: 'positive-definite-challenge',
      title: 'Build Positive Definite',
      description: 'Create a symmetric matrix where both eigenvalues are strictly positive.',
      component: 'EigenTransform',
      completionCriteria: { type: 'threshold', target: 0.1, metric: 'min_eigenvalue' },
      hints: [
        'Symmetric means b = c. Positive definite needs det > 0 and trace > 0.',
        'Try [[3,1],[1,3]] — both eigenvalues are 4 and 2.',
      ],
    },
    {
      id: 'fast-convergence',
      title: 'Power Iteration Speedrun',
      description: 'Set a matrix where power iteration converges in ≤ 3 iterations (eigenvalue ratio |λ₁/λ₂| > 5).',
      component: 'EigenTransform',
      completionCriteria: { type: 'threshold', target: 5, metric: 'eigenvalue_ratio' },
      hints: [
        'The larger the ratio |λ₁/λ₂|, the faster convergence.',
        'Try [[10,0],[0,1]] — ratio is 10:1, converges almost instantly.',
      ],
    },
  ],
};
